...
const NewComparison = (props) => {
  ...
  const createNewComparison = (event) => {
    ...
    if (
      props.selectedPrograms.length < 2 ||
      categories.length === 0 ||
      reviewOne.length === 0 ||
      reviewTwo.length === 0
    ) {
    ...
    fetch(props.API + "comparisons", postConfig)
      ...
      .then((newCompObject) => {
        props.selectedPrograms.forEach((programObject, index) => {
          ...
          programObject.rating =
            (programObject.rating + ratings[index]) /
            programObject.comparisonsRef.length;
          let patchConfig = {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
            ...
            body: JSON.stringify({
              comparisonsRef: programObject.comparisonsRef,
              rating: programObject.rating,
            }),
          };
            ...
            <Card.Group itemsPerRow={2}>
              <Card>
                ...
                <Rating
                  ...
                  defaultRating={0}
                  maxRating={5}
                  onRate={(e, { rating }) => setRatingOne(rating)}
                />
              </Card>
              <Card>
                <Form.Field
                ...
                <Rating
                  ...
                  defaultRating={0}
                  maxRating={5}
                  onRate={(e, { rating }) => setRatingTwo(rating)}
                />
              </Card>
            </Card.Group>
            <Message
